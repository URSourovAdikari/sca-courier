...
function EditNameModal({ currentName, onClose, onSuccess, session, update }: {
  ...
  return (
    <Modal onClose={onClose}>
      <div className="px-7 pt-7 pb-4 border-b border-neutral-100 dark:border-neutral-800 flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-neutral-900 dark:text-white tracking-tight">Update Name</h2>
            <p className="text-xs text-neutral-400 font-medium">Changed your mind?</p>
          </div>
        </div>
        <button onClick={onClose} className="p-2 text-neutral-400 hover:text-neutral-900 dark:hover:text-white hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-xl transition-all">
          <X size={20} />
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="px-7 py-8 flex flex-col gap-6">
        {error && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            className="flex items-center gap-3 p-4 rounded-2xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 text-red-600 dark:text-red-400 text-sm font-medium">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="relative group">
            <User size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-400 group-focus-within:text-indigo-500 transition-colors pointer-events-none" />
            <input
              value={name}
              ...
              autoFocus
              className="w-full pl-12 pr-4 py-4 rounded-2xl text-base border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-900/40 text-neutral-900 dark:text-white placeholder:text-neutral-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 dark:focus:border-indigo-500 transition-all disabled:opacity-50"
            />
          </div>
        </div>
        ...
        <div className="flex gap-4 pt-2">
          <button type="button" onClick={onClose} disabled={loading}
            className="flex-1 py-4 rounded-2xl border border-neutral-200 dark:border-neutral-700 text-base font-bold text-neutral-600 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all disabled:opacity-50">
            Keep Old
          </button>
